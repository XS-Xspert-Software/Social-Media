...
function looksValidKeyId(k: string) { return /^00[0-9a-f]{22,}$/i.test(k); }
function looksValidBucketId(b: string) { return /^[0-9a-f]{10,}$/i.test(b); }
...
async function ensureAuthorized() {
  ...
  if (now - lastAuthTime < AUTH_TTL_MS) return; // still fresh
  authInFlight = (async () => {
    try {
      await b2.authorize();
...
export interface UploadedVideoInfo {
  ...
  size: number;
  mimeType: string;
  downloadUrl?: string; // If bucket is public this can be constructed
}

export async function uploadVideoToB2(filePath: string, fileName: string): Promise<UploadedVideoInfo> {
  if (!B2_KEY_ID || !B2_APP_KEY || !B2_BUCKET_ID) {
