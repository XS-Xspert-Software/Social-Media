...
import { getTrustedServers } from './utils/config.js';
...
app.post('/api/video/upload', upload.single('video'), wrapAsync(async (req, res) => {
    if (!req.file) {
...
app.get('/api/user/settings', wrapAsync(async (req, res) => {
...
app.post('/api/user/settings', wrapAsync(async (req, res) => {
...
app.post('/api/user/preferences', wrapAsync(async (req, res) => {
...
async function proxyRemoteFetch(url, res) {
    try {
...
app.post('/api/admin/verify', wrapAsync(async (req, res) => {
